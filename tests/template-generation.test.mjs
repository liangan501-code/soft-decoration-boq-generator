...
test("current-space contextual generation has complete exact living-room rows", () => {
  const contextualStart = source.indexOf("function getContextualTemplateRows");
  const contextualEnd = source.indexOf("function getTemplateRows", contextualStart);
  const contextualRows = source.slice(contextualStart, contextualEnd);
  ...
  for (const name of [
    ...
    assert.match(contextualRows, new RegExp(name), `${name} should be generated by contextual current-space templates`);
  }

  assert.match(source, /if \(getContextualTemplateRows\(space, style, context\)\) return baseName;/);
  assert.match(source, /const contextualRows = getContextualTemplateRows\(space, style, context\);/);
});

test("generation buttons stay clickable so missing uploads can show toast", () => {
  assert.doesNotMatch(source, /button\.disabled = !ready;/);
  assert.match(source, /button\.disabled = false;/);
  assert.match(generationSource, /showToast\(REQUIRED_UPLOAD_MESSAGE\);/);
});
